...
2. **Read the design system first.** `design-system/DESIGN.md` — at the vault root, before and after bootstrap — for tokens, the fixed type scale, mark budget and safe areas, and `design-system/RUBRIC.md` for the ship bar. Deviations are allowed but must be argued, not accidental. If no design system exists yet, say so once — an inconsistent brand is a Layer 3 problem, not a note on this asset.
