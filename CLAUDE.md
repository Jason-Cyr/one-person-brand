...
`.claude/skills/` holds `content-research` (what to make, with sources and confidence marks), `fact-check` (run it before the record or send button, not after), and `design-system-setup` (build `DESIGN.md` with the human, once).

See the *critic bench* and *skills* sections of `AGENTS.md` for how to run all of these and how to keep them sharp. Never move or delete `.claude/` when tidying the vault.
